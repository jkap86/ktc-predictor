...
Usage:
    cd backend
    python -m iterations.v1_hgb_baseline.train --zip data/training-data.zip --out iterations/v1_hgb_baseline/models
"""
...
def _ktc_tier_key(start_ktc: float) -> str | None:
    """Return the tier key string for a start_ktc value, or None."""
    for lo, hi in KTC_TIER_BUCKETS:
        if lo <= start_ktc < hi:
            return f"tier_{lo}_{hi}"
    return None
...
def train_all(
    ...
    for pos in POSITIONS:
        ...
        if not no_calibration and not is_linear:
            ...
            # Third-stage: per-KTC-tier calibration
            # Corrects systematic bias at different value tiers (e.g. TE 6k+ underprediction)
            # Skipped for positions where it hurts (RB)
            start_ktc_train_vals = start_ktc[train_idx]
            start_ktc_test_vals = start_ktc[test_idx]

            # Apply pos_cal to OOF predictions for tier calibrator fitting
            oof_preds_pos = pos_cal.predict(oof_preds_cal)
    ...
    # Save per-position start_ktc p95 for sentinel imputation at inference
    # Must match data.py: sentinels are elite ceiling players, not average
    sentinel_impute = {}
    for pos in POSITIONS:
        pos_df = df[df["position"] == pos]
...
def main():
    ...
    parser.add_argument(
        "--out",
        default="iterations/v1_hgb_baseline/models",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Minimum season year to include in training data (inclusive)",
    )
    parser.add_argument(
        "--max-year",
        type=int,
        default=None,
        help="Maximum season year to include in training data (inclusive)",
    )
