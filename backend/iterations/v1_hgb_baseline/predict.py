...
from .age_adjustment import apply_age_decline_adjustment, env_flag
...
def get_expected_features(position: str) -> list[str]:
    ...
    # Prior-season features — included for all positions in ablation
...
def apply_extreme_shrinkage(
    log_ratio: float,
    threshold: float = 0.7,
    decay: float = 0.5,
) -> float:
    """Dampen extreme predictions toward zero.

    Beyond the threshold, additional magnitude is scaled by decay factor.
    This prevents unrealistic 300%+ swings from low-KTC players.

    Example: threshold=0.7, decay=0.5
      log_ratio=0.5  → 0.5 (below threshold, unchanged)
      log_ratio=1.5  → 0.7 + (1.5-0.7)*0.5 = 1.1 (dampened from +349% to +200%)
      log_ratio=-1.0 → -(0.7 + (1.0-0.7)*0.5) = -0.85 (dampened)
    """
    sign = 1.0 if log_ratio >= 0 else -1.0
    magnitude = abs(log_ratio)
    if magnitude <= threshold:
        return log_ratio
    dampened = threshold + (magnitude - threshold) * decay
...
def predict_end_ktc(
    ...
    **_unused_kwargs,
    ...
    # Prior-season KTC features for all positions
    if position in ("QB", "RB", "WR", "TE"):
    ...
    # Shrink extreme predictions toward zero (prevents 300%+ swings)
