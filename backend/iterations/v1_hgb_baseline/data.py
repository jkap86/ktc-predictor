...
def build_weekly_snapshot_df(
                ...
                rows.append(
                    {
                        ...
                        # ── v2 feature candidates (season-level from raw data) ──
                        # boom_rate: % of weeks with top-quartile FP (upside frequency)
                        "boom_rate": season.get("boom_rate", 0) or 0,
                        # bust_rate: % of weeks with bottom-quartile FP (downside risk)
                        "bust_rate": season.get("bust_rate", 0) or 0,
                        # weekly_fp_cv: coefficient of variation in weekly FP (consistency)
                        "weekly_fp_cv": season.get("weekly_fp_cv", 0) or 0,
                        # start_position_rank: dynasty position ranking at season start
                        "start_position_rank": season.get("start_position_rank"),
                        # last_4_vs_season: last-4-weeks FP ratio vs season avg (form)
                        "last_4_vs_season": season.get("last_4_vs_season"),
                        "has_last4_data": 1 if season.get("last_4_vs_season") is not None else 0,
    ...
    for pos in VALID_POSITIONS:
        ...
        if not mask.any():
            continue
        # start_ktc sentinels → p95 (these are elite ceiling players, not average)
        start_sent = mask & (df["start_ktc"] >= 9999)
        if start_sent.any():
            non_sent = mask & (df["start_ktc"] < 9999)
            df.loc[start_sent, "start_ktc_was_sentinel"] = 1
            df.loc[start_sent, "start_ktc"] = df.loc[non_sent, "start_ktc"].quantile(0.95)
