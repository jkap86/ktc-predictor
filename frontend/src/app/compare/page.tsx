...
function CompareContent() {
  ...
  useEffect(() => {
    ...
    const newUrl = selectedIds.length > 0 ? `?${params.toString()}` : '/compare';
  ...
  }, [predictions, whatIfGames, whatIfPpg]);
          ...
          {whatIfLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
              {whatIfResults.map((result, i) => {
                ...
                return (
                  <div key={predictions[i]?.player_id || i} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div className="font-medium text-sm text-gray-900 dark:text-white mb-1 truncate">
                      {predictions[i]?.name}
                    </div>
                    <div className="text-sm">
                      <span className="text-gray-500 dark:text-gray-400">EOS: </span>
